...
import { ArrowRight, Layout, Cpu, Zap, Shield, Globe, Box, Layers, MousePointer2, Smartphone, Type, Monitor } from "lucide-react";
  ...
  const features = [
    {
      ...
      description: "Building scalable systems from the smallest atom to the most complex organism for absolute consistency.",
      icon: Box,
      color: "#6324FC"
    },
    {
      title: "Design Tokens",
      description: "Defining shared styles as platform-agnostic tokens (colors, spacing, type) that sync instantly across dev/design.",
      icon: Layers,
      color: "#00E5FF"
    },
    {
      title: "Component Libraries",
      description: "Curating a library of high-performance, reusable UI components that slash development time by 50%+.",
      icon: Layout,
      color: "#FF2D55"
    },
    {
      title: "Living Documentation",
      description: "Comprehensive usage guidelines and accessibility standards that evolve with your product ecosystem.",
      icon: Globe,
      color: "#00FFA3"
    }
  ];
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-20 items-center mb-48">
          <div>
            ...
            >
              ...
              <span className="font-mono text-[#6324FC] text-[10px] tracking-[0.5em] uppercase">Systems Division</span>
            </motion.div>
            
            <h1 className="font-heading text-8xl md:text-[9vw] leading-[0.85] tracking-tighter text-white uppercase mb-16">
              DESIGN <br/>
              <span className="stroke-text text-transparent italic opacity-40">SYSTEMS.</span>
            </h1>
            ...
            <p className="font-sans font-light text-2xl md:text-3xl text-white/40 max-w-2xl leading-relaxed border-l-2 border-[#6324FC] pl-8 mb-16">
              We engineer scalable visual languages. Our design systems bridge the gap between design and development — ensuring speed, consistency, and absolute brand integrity.
            </p>
        ...
        <div className="mb-48">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-24">
            <h2 className="font-heading text-6xl text-white uppercase tracking-tighter max-w-xl">
              CONSISTENCY <span className="italic text-[#6324FC]">AT SCALE.</span>
            </h2>
            <p className="font-mono text-[11px] tracking-widest text-white/30 uppercase max-w-xs leading-loose">
              Stop reinventing the wheel. Invest in a system that allows your team to build faster and maintain a unified brand voice.
            </p>
          </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div>
                ...
                <p className="font-sans font-light text-xl text-white/40 mb-12 leading-relaxed">
                   We follow Brad Frost&apos;s atomic design principles to build modular components that are durable, testable, and reusable.
                </p>

                <div className="space-y-6">
                   {[
                     { label: "Atoms", desc: "Colors, Icons, Typography, Input Fields." },
                     { label: "Molecules", desc: "Search Bars, Card Headers, List Items." },
                     { label: "Organisms", desc: "Navbars, Footers, Data Grids, Forms." }
                   ].map((item, i) => (
                     <div key={i} className="flex gap-6 group">
                        <div className="w-1.5 h-1.5 rounded-full bg-[#6324FC] mt-2 group-hover:scale-150 transition-transform" />
                        <div>
                           <div className="font-heading text-xl text-white uppercase">{item.label}</div>
                           <p className="font-sans text-white/30 text-sm">{item.desc}</p>
                        </div>
                     </div>
                   ))}
                </div>
              ...
              <div className="relative aspect-square bg-[#0A0A0C] border border-white/10 rounded-3xl p-10 shadow-2xl shadow-black overflow-hidden group">
                 ...
                 <div className="relative z-10 h-full flex flex-col items-center justify-center gap-12">
                    ...
                    >
                       ...
                       <motion.div animate={{ x: [-20, 0, -20] }} className="absolute -top-4 -left-4 w-12 h-12 bg-white/10 border border-white/20 rounded-xl flex items-center justify-center">
                          <MousePointer2 className="w-5 h-5 text-[#00E5FF]" />
                       </motion.div>
                       <motion.div animate={{ x: [20, 0, 20] }} className="absolute -bottom-4 -right-4 w-12 h-12 bg-white/10 border border-white/20 rounded-xl flex items-center justify-center">
                          <Type className="w-5 h-5 text-[#FF2D55]" />
                       </motion.div>
                    </motion.div>
                    
                    <div className="text-center space-y-2">
                       <div className="font-mono text-[10px] tracking-[0.5em] text-[#6324FC] uppercase">System Assembly</div>
                       <div className="font-heading text-2xl text-white">COMPONENT-DRIVEN</div>
                    </div>
                 </div>
              </div>
           </div>
        ...
        <div className="mb-48">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              {[
                { step: "01", title: "Audit & Align", desc: "Analyzing your current UI debt and aligning on a unified visual direction for the entire organization." },
                { step: "02", title: "System Build", desc: "Creating the component library and design tokens in Figma and synchronizing them with React/Next.js code." },
                { step: "03", title: "Documentation", desc: "Deploying a living style guide that serves as the single source of truth for design, engineering, and product." }
              ].map((s, i) => (
        ...
        <div className="text-center py-24 border-t border-white/5">
          ...
          <p className="font-sans text-white/40 text-2xl mb-16 max-w-2xl mx-auto leading-relaxed">
            Ready to build a scalable future? Let&apos;s architect a design system that empowers your team to ship faster and better.
          </p>
          <motion.button 
