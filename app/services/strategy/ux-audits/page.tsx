...
import { ArrowRight, Search, Zap, Shield, Globe, Eye, MousePointer2, UserCheck, Layers, ClipboardCheck, Activity, Smartphone } from "lucide-react";
  ...
  const features = [
    {
      ...
      description: "Analyzing your interface against the 10 core principles of interaction design to find hidden friction points.",
      icon: Eye,
      color: "#6324FC"
    },
    {
      title: "Accessibility Compliance",
      description: "Ensuring your digital product meets WCAG standards and is inclusive for all users, regardless of ability.",
      icon: UserCheck,
      color: "#00E5FF"
    },
    {
      title: "User Journey Mapping",
      description: "Visualizing the entire customer path to identify and eliminate barriers to conversion and satisfaction.",
      icon: Layers,
      color: "#FF2D55"
    },
    {
      title: "Performance UX",
      description: "Measuring the psychological impact of page load speeds and interaction latency on your user retention.",
      icon: Activity,
      color: "#00FFA3"
    }
  ];
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-20 items-center pt-32 pb-16 md:pt-40 md:pb-24 border-b border-white/5">
          <div>
            ...
            >
              ...
              <span className="font-heading  text-[#6324FC] text-[10px] tracking-[0.5em] capitalize ">Experience Division</span>
            </motion.div>
            ...
            <p className="font-sans font-light text-2xl md:text-3xl text-white/40 max-w-2xl leading-relaxed border-l-2 border-[#6324FC] pl-8 mb-16">
              We audit for empathy. Our UX experts deconstruct your product to reveal the hidden frustrations preventing your users from falling in love with your brand.
            </p>
          ...
          >
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center">
               ...
               <div className="font-heading  text-[10px] tracking-[0.5em] text-white/60 capitalize ">Empathy Scanned</div>
            </div>
          </motion.div>
        </div>
        ...
        <div className="py-16 md:py-24 border-b border-white/5">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-24">
            <h2 className="font-heading text-5xl md:text-7xl leading-[0.85] tracking-tighter text-white capitalize max-w-xl">
              ELIMINATING <span className="italic text-[#6324FC]">FRICTION.</span>
            </h2>
            <p className="font-heading  text-[11px] tracking-widest text-white/30 capitalize max-w-xs leading-loose">
              A great user experience is invisible. We find the visible problems and turn them into seamless interactions.
            </p>
          </div>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
              <div className="relative aspect-square bg-[#0A0A0C] border border-white/10 rounded-3xl p-10 shadow-2xl shadow-black overflow-hidden flex flex-col justify-between group">
                 <div className="flex justify-between items-start">
                    ...
                    <div className="font-heading  text-[9px] text-white/20 capitalize tracking-[0.3em]">Behavioral Node // 0xUX</div>
                 </div>

                 <div className="flex-1 flex flex-col justify-center space-y-8">
                    {[
                      { label: "Navigation Flow", val: "Optimized", color: "text-[#00E5FF]" },
                      { label: "Interaction Debt", val: "Reduced", color: "#6324FC" },
                      { label: "Visual Hierarchy", val: "Balanced", color: "text-[#00FFA3]" }
                    ].map((item, i) => (
                      <div key={i} className="flex items-center justify-between border-b border-white/5 pb-4 group-hover:border-[#6324FC]/30 transition-colors">
                         <div className="font-heading text-2xl text-white capitalize ">{item.label}</div>
                         <div className={`font-heading  text-[10px] tracking-widest capitalize ${typeof item.color === 'string' && item.color.startsWith('text') ? item.color : ''}`} style={typeof item.color === 'string' && !item.color.startsWith('text') ? { color: item.color } : {}}>{item.val}</div>
                      </div>
                    ))}
                 </div>
              ...
              <div>
                ...
                <p className="font-sans font-light text-xl text-white/40 mb-12 leading-relaxed">
                   We look at your product through the eyes of your users. Our audits identify not just what is broken, but why users feel the way they do.
                </p>

                <div className="space-y-8">
                   {[
                     { label: "Pain Point Discovery", desc: "Pinpointing the exact moments when users feel frustrated or confused." },
                     { label: "Retention Strategy", desc: "Building the interaction hooks that keep users coming back for more." },
                     { label: "Cross-Device Audit", desc: "Ensuring your experience is flawless from the smallest mobile screen to the largest monitor." }
                   ].map((item, i) => (
        ...
        <div className="py-16 md:py-24 border-b border-white/5">
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              {[
                { step: "01", title: "Inspection", desc: "Deep-diving into every screen and interaction to map out the current state of your user experience." },
                { step: "02", title: "Analysis", desc: "Synthesizing our findings into a prioritized roadmap of design and technical improvements." },
                { step: "03", title: "Evolution", desc: "Providing the high-fidelity prototypes and specs needed to execute the transformation." }
              ].map((s, i) => (
        ...
        <div className="text-center py-16 md:py-24 border-t border-white/5">
          ...
          <p className="font-sans text-white/40 text-2xl mb-16 max-w-2xl mx-auto leading-relaxed">
            Ready to fix the friction? Let&apos;s run a surgical UX audit and build an experience that your users will never want to leave.
          </p>
          <motion.button 
