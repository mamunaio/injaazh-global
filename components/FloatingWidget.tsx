  ...
  return (
    <div className="fixed bottom-4 right-4 md:bottom-8 md:right-8 z-[100] flex flex-col items-end gap-3 md:gap-4">
      ...
      >
        ...
        <button className="group relative p-[1.5px] rounded-full overflow-hidden flex items-center justify-center cursor-pointer transition-all duration-300 active:scale-95 hover:shadow-[0_0_30px_rgba(99,36,252,0.3)]">
          ...
          <div className="relative w-12 h-12 md:w-14 md:h-14 rounded-full bg-[#F5F5F0] dark:bg-[#060608] transition-colors duration-500 flex items-center justify-center z-10">
            {/* Flowing background gradient */}
            <div className="absolute inset-0 bg-gradient-to-r from-[#6324FC]/10 to-[#00E5FF]/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-full" />

            {mounted ? (
              theme === "light" ? (
                <Sun className="w-5 h-5 text-[#6324FC] relative z-10" />
              ) : theme === "dark" ? (
                <Moon className="w-5 h-5 text-[#6324FC] relative z-10" />
              ) : (
                <Monitor className="w-5 h-5 text-[#6324FC] relative z-10" />
              )
            ) : (
      ...
      <AnimatePresence>
        {showScrollTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="w-12 h-12 md:w-14 md:h-14 bg-[#6324FC] flex items-center justify-center text-white shadow-[0_15px_35px_rgba(99,36,252,0.3)] hover:bg-[#5219d6] transition-all duration-500 rounded-full group"
          >
